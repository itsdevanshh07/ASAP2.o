...
export const createResume = async (req, res) => {
    const { userNotes, resumeData: manualData, template = 'combination' } = req.body;

    if (!userNotes && !manualData) {
        return res.status(400).json({ error: "User notes or resume data required" });
    }
    ...
    try {
        ...
        if (manualData) {
            ...
            resumeData = manualData;
        } else {
            // 1. Get structured data from AI
            resumeData = await generateResume(userNotes);
        }
